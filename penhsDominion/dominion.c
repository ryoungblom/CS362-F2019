...
int ambassadorCardEffect(int choice1, int choice2, struct gameState *state, int handPos, int currentPlayer) {
...
int tributeCardEffect(struct gameState *state, int currentPlayer, int nextPlayer) {
	...
	if ((state->discardCount[nextPlayer] + state->deckCount[nextPlayer]) <= 1) {
	    ...
	    else if (DEBUG) {	//No Card to Reveal
	...
	if (tributeRevealedCards[0] == tributeRevealedCards[2]) { //If we have a duplicate card, just drop one
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
    ...
    case mine:
    	mineCardEffect(choice1, choice2, state, handPos, currentPlayer);
        return 0;
    ...
    case minion:
        minionCardEffect(choice1, choice2, state, handPos, currentPlayer);
        return 0;
    ...
    case tribute:
    	tributeCardEffect(state, currentPlayer, nextPlayer);
        return 0;

    case ambassador:
    	ambassadorCardEffect(choice1, choice2, state, handPos, currentPlayer);
        return 0;
